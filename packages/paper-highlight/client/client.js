window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const styles = {
			insert(css) {
				const tagId = "paper-highlight/client.css";
				if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
					const tag = document.createElement("style");
					tag.dataset.plugin = "paper-highlight";
		...
		const callData = async (q) => {
			const url = "/paper-hl/read" + (q && q.paperId ? "?paperId=" + encodeURIComponent(q.paperId) : "");
			const res = await fetch(url, { headers: { Accept: "application/json" } });
			if (!res.ok) throw new Error("GET " + url + " -> " + res.status);
			return res.json();
		};
		const writeData = async (url, body) => {
			const res = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json", Accept: "application/json" }, body });
			if (!res.ok) throw new Error("POST " + url + " -> " + res.status);
			return res.json();
		};
		const profileData = async (method, url, body) => {
			const headers = { Accept: "application/json" };
			const opts = { method, headers };
			if (method !== "GET" && body != null) { headers["Content-Type"] = "application/json"; opts.body = body; }
		...
		const proposeData = async (url, body) => {
			const res = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json", Accept: "application/json" }, body });
			if (!res.ok) throw new Error("POST " + url + " -> " + res.status);
			return res.json();
		};
		...
		exports.buildBlockSegments = buildBlockSegments;
		exports.buildSegmentMap = buildSegmentMap;
		...
		exports.reviewProgress = reviewProgress;
		exports.buildExportUrl = buildExportUrl;
		exports.localApproveSectionSpans = localApproveSectionSpans;
		exports.localRevertSectionSpans = localRevertSectionSpans;
		exports.colorLegend = colorLegend;
		exports.callProfile = callProfile;
		exports.callFormat = callFormat;
		exports.callProposeRequest = callProposeRequest;
		...
		exports.proposalCardModel = proposalCardModel;
		exports.buildApplyDecisions = buildApplyDecisions;
