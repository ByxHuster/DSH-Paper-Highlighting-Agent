...
> **v0.6.1（重新完成行内数学公式渲染 + KaTeX 升级）2026-09-01 实现 PASS（待归档 `v0.6.1`）**：在 v0.5.4.3 代码树上**重建行内公式渲染**（v0.5.1 实现 → v0.5.3 回滚后本次重做；数据零改动；**零 CDN 依赖 + 本地 KaTeX**，无任何外部网络）。**管线**：`splitMathPieces`（识别裸 LaTeX 片段，散文词边界停止）→ `repairMath`（MinerU OCR 空格污染修复：A1 花括号收拢 / A2 `\cmd {` 紧贴 / A3/A3b 上下标空格 / A4 空格打散命令重建 `l o g`→`log` / A5 `i . e .` 句点修复 / A7 `2 0 1 1`→`2011` / A6 `~`、`\:`→空格）→ **`katexRender`（KaTeX 渲染，LaTeX 级排版 + 自带数学字体无方框；抛错/缺失降级回 Unicode+CSS 近似保底）**。**渲染集成**：`katex.min.js` + 20 个数学字体 base64 内联进 bundle（117KB→759KB，本地加载无网络，无需重启 3081）；segment 新增 `math`/`dlen`（显示长度）字段，`renderText` 输出 `.phl-math` span；选区经 `data-phl-dlen` 桥接回原文偏移，数学段整段映射；**G2 防复犯**：空/纯空白显示折叠回纯文本（杜绝 v0.5.2 灰块）；**G3**：非数学正文逐字节不变。**验证**：`run-render.js` 数学离线矩阵（repair/split/convert/KaTeX 分支/降级分支/dlen/选区映射/G2/G3 + G1 嵌入完备性 32 helpers）全绿 + 全回归套件（run-actions/run-plugin/run-export/run-format/run-profile/run-mock/run-real/run-tools）PASS + `simulate-render` 数学/KaTeX 静态守卫绿（E2E 数据断言受演示数据 0 spans 阻塞属 v0.6.0 既有状态）+ **`run-math-g4.js` 真实 bundle × 3 篇全锚点审计 PASS**（293 锚点 / 148 数学段 / 0 崩溃 / 0 空显示 / **KaTeX 148/148 接管 / 0 katex-error**）。纯 client 改动，**无需重启 3081，刷新页面即生效**。进度见 `D:\aa\docs\paper-highlight-progress-v0.6.md`。
>
> **v0.6.1.2（修复：数学区误吞散文括注）2026-09-01**：修复 `splitMathPieces` 两处误判——① `parseMathRight` 的 i++ 双增 bug（闭括号使 depth 未归零时 i 被加两次、跳过相邻 `))` 的第二个 `)` → depth 泄漏、数学区吞到句尾）；② 括号内散文括注（如 `s _ { i - 1 } (just before emitting y _ { i } , Eq. (4)) …` 的 `(just … Eq. (4))`）被 depth 机制整体吞为数学。修复：闭括号统一单增 + depth 内散文词退出（`(just` 这种 `(` 后跟散文词不再开深度）+ 命令参数豁免（`\end{array}` 的 `{array}` 是标识符不是散文，否则 `\end` 被截断 → KaTeX 报错）。**验证**：run-render 新增 aside/嵌套 `))` 回归断言 + G4 全量回到 293 锚点 / 151 数学段 / **0 katex-error**（此前 1）+ 全回归套件绿。纯 client，重新生成 bundle（760KB）后刷新即生效。进度见 `D:\aa\docs\paper-highlight-progress-v0.6.md`。
>
> **v0.6.2（行间公式：保留 MinerU 公式块 + 块级 KaTeX 渲染）2026-09-01**：不再依赖文本内检测（`$$`/环境块）——`normalize.js` 的 `KEEP_TYPES` 加入 `interline_equation`/`formula`（公式块作为普通 anchor，type 保留，跳过 header/footer 启发式）；client 对公式类型 anchor 走**块级 KaTeX 渲染**（`katexRender(tex, {displayMode:true})` → `.phl-math-display` 居中块 + 浅蓝标记 + G2 空折叠），选区/高亮偏移机制 type-agnostic 零改动，propose 的 read_section 自动包含公式原文。**真实数据验收（B 路线：重解析 sutskever）**：MinerU 云 API 重解析（`enable_formula` 已默认开）→ 4 个 `interline_equation` 块保留（70 anchors：`\begin{array}{r c l}` 矩阵、`\prod`、`\sum`、`\tag{2}`），KaTeX displayMode 渲染 0 katex-error。**G4**：297 锚点 / 155 数学段 / 0 katex-error；run-mock 适配（mock 公式块现为第 5 锚点）；全回归绿。sutskever 数据为有意重解析变更（0 spans 无高亮损失）；mikolov/bahdanau 维持现状。纯 client + 数据，刷新 GUI 看 sutskever 第 3-4 页公式块即验收。进度见 `D:\aa\docs\paper-highlight-progress-v0.6.md`。
>
> **v0.6.3（保留表格 + 图/表标题；图片主体留第二步）2026-09-01**：MinerU 表格是**完整 HTML**（表格识别）——`normalize.js` 的 `KEEP_TYPES` 加 `table`/`table_body`/`table_caption`/`image_caption`/`chart_caption`，`walkPages` **展开嵌套容器块**（image/table/chart → body/caption 子块，展平唯一 block 编号）；表格 anchor 的 `text` = HTML 的可读文本化（`htmlToPlain`：行分隔 + `|` 单元格 + 实体解码，保 md_offset 契约/propose 可读性），原始 HTML 存 `anchor.html`；client 对表格 anchor 渲染真实 HTML 表格（`.phl-table` + 边框样式 + 无高亮 by design），标题渲染为 `.phl-caption` 斜体小注。**真实数据验收**：复用 v0.6.2 的 MinerU zip 重归一化 sutskever → **79 anchors**（+9 = 3 表格 + 3 表格标题 + 1 图标题 + 2 图表标题），`image_body`/`chart_body`（图片主体）仍跳过留待 v0.6.4。**G4**：306 锚点 / 155 数学段 / 0 katex-error；run-mock 适配（mock 扁平 table 保留为第 6 锚点）；simulate 加 `.phl-table`/`.phl-caption` 守卫；全回归绿。纯 client + 数据，刷新 GUI 看 sutskever 第 6-7 页 3 个真实表格（BLEU 对比 + 长句翻译示例）即验收。进度见 `D:\aa\docs\paper-highlight-progress-v0.6.md`。
>
> **v0.6.4（保留图片/图表主体：host 静态路由 + 真实 `<img>` 渲染）2026-09-01**：A1 路线——`normalize.js` `KEEP_TYPES` 加 `image_body`/`chart_body`，从 zip 提取 `spans[].image_path` 指向的栅格字节（`images[]` 随产物返回），anchor 记相对路径 `img: 'images/<name>'` + 占位 `text: '[figure]'/'[chart]'`（保 md_offset 契约）；`store.js` `writePaper` 将栅格落盘 `data/<paper_id>/images/`；**`plugin.js` 新增 `GET /paper-hl/images/<paperId>/<name>` 静态路由**（严格 id/name 字符集校验防目录穿越 + 404 + image/jpeg）；client 对 image/chart 主体渲染 `<img src="/paper-hl/images/...">`（`.phl-figure` 居中/圆角/懒加载，无高亮 by design）。**真实数据验收**：sutskever 重归一化 → **84 anchors**（+5 = 1 图 Figure 1 架构图 998×212 + 4 图表），**5 张 JPEG 落盘**（85KB，全部真实可解码）；G4 311 锚点 / 155 数学段 / 0 katex-error；run-mock 嵌套 image 容器单测（展开 + 栅格提取 + 落盘）；run-plugin 图片路由矩阵（200/400 穿越/400 非法名/404）；simulate 加 `.phl-figure` 守卫；全回归绿。**注意：需重启 3081 使图片路由生效**（本次是首个涉及 host 改动的版本；重启后 parse_pdf 亦直接用新 normalize）。纯 client + host + 数据。进度见 `D:\aa\docs\paper-highlight-progress-v0.6.md`。
>
> **v0.7.0（文档 + propose 显式跳过表格/图片）2026-09-01**：三份文档（`docs/paper-highlight-user-guide.md` 用户指南、`docs/paper-highlight-report-01-dsh.md` DSH 生态与嵌入、`docs/paper-highlight-report-02-agent.md` Agent 工作流程与结构）。**propose 显式跳过展示性内容**：表格/图片主体 anchor（type ∈ `{table, table_body, image_body, chart_body, image, chart}`）不参与高亮——`skills/propose.md` 与 `skills/global-read.md` 加显式规则（仅阅读上下文，标题 `*_caption` 可正常高亮），`schema.js` 加**硬兜底** `NON_HIGHLIGHTABLE_TYPES`（违规 spans 写回被拒，绝不静默丢弃），run-mock 断言兜底（image_body/table 上提 spans 被拒）。全回归绿。纯规则 + 校验，无数据变更。
...
host/
  ...
  plugin.js     # (Step 3) durable host 插件：/paper-hl/read + /write + /profile + /init + /apply + /save + /export + /format + /propose-request 路由
...
client/
  ...
  render-body.js# 渲染逻辑单一来源（gen-client.js 由它生成 bundle 与动态半；keyAction/reviewProgress/buildExportUrl/callFormat + v0.5.1 图例彩色语义 + **v0.6.1 行内数学渲染**：repairMath/splitMathPieces/mathConvert/katexRender/segLen/buildTextPieces 纯函数，renderText/buildBlockSegments/mapSelection 数学片段 + data-phl-dlen 偏移桥接；KaTeX 运行时与字体由 gen-client 内联进 bundle）
scripts/
  gen-client.js # 生成 client/client.js 与 dynamic/client-half.js（v0.6.1+ 内联 KaTeX 运行时 + 数学字体 base64）
...
test/
  ...
  run-render.js # 渲染纯函数矩阵（P2-a…e + v0.3 colorLegend/callProfile/面板模型/提案模型 + v0.4 keyAction/reviewProgress/buildExportUrl + v0.5 callFormat + v0.6.1 数学管线矩阵：repairMath/splitMathPieces/mathConvert/katexRender（KaTeX 分支 + 降级分支）/segLen/buildTextPieces/选区 dlen 映射/G2 空显示折叠/G3 非数学逐字节回归/G1 嵌入完备性）
...
- client bundle：`client/client.js`（`fetch('/paper-hl/read')`，锚点序渲染 + `<mark>` 高亮 + 图例（**v0.5.1 语义标签彩色**）+ 刷新/选论文/导出/格式化 + **v0.6.1 行内数学渲染**（本地 KaTeX：`katex.min.js` + 数学字体 base64 内联，LaTeX 级排版无方框；OCR 修复层；导出仍保留原始文本））
