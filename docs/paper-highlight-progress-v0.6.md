...
> 版本：v0.6.x（计划文档）· 状态：**v0.6.1 行内公式已实现并验收 PASS（2026-09-01）—— v0.6.2 行间公式待实现** · 创建：2026-09-01
...
| D3 | **零依赖**：不引入 KaTeX / MathJax / 任何外部资源；Unicode 符号 + 组合附加符 + CSS | 离线可用、可无头测试，沿用项目约束 |
...
| G4 | 真实数据全量渲染 | 3 篇论文全锚点渲染 0 错误 + 0 灰块 + 转换前后 diff 抽样 | 端到端质量 |
...
| 版本 | 内容 | 验收 |
|---|---|---|
| v0.6.1 | 行内公式：修复层 + 分段层 + 转换层 + 渲染集成 + 回归闸门 | ✅ **已实现并 PASS**：`run-render` 离线矩阵全绿（repair/split/convert/dlen/选区映射/G2/G3/G1-31 helpers）+ 全回归套件绿 + `simulate-render` 数学静态守卫绿（E2E 数据断言受演示数据 0 spans 阻塞为 v0.6.0 既有状态）+ `run-math-g4` 真实 bundle × 3 篇全锚点审计 PASS（293 锚点 / 148 数学段 / 0 崩溃 / 0 空显示） |
